...
public class HudState extends BaseAppState implements ActionListener {
	...
	public void update(float tpf) {
		...
		if (isMoving) {
			...
			// 判断当前方向是在原方向的左侧还是右侧
			Vector3f oldLinear = head.getLinear();
			...
			if (FastMath.abs(tmp) > 0.000001f) {
				...
				// 线速度
				head.getLinear().set(dir.x, dir.y, 0);
			}
		...
		// 跟踪镜头
		Vector3f loc = head.getLocation();
		cam.setLocation(new Vector3f(loc.x, loc.y, 300));
		
		// 状态值
		lengthTxt.setText("长度: " + player.getLength());
		int kill = 0;
